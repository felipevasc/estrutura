...
import { detectarLinguagens, coletarRespostaTecnologia, registrarLogResposta } from './identificadoresTecnologia';
...
export class IdentificarLinguagemService extends NanoService {
  constructor() {
    super('IdentificarLinguagemService');
  }

  initialize(): void {
    this.listen(NanoEvents.COMMAND_RECEIVED, (payload: any) => {
      if (payload.command === 'identificarLinguagem') this.processar(payload as PayloadComando);
    });
  }

  private async processar(payload: PayloadComando) {
    const { id, args } = payload;
    const comandoRegistrado = registrarComandoFerramenta('identificarLinguagem', id, 'identificarLinguagem', [JSON.stringify(args ?? {})]);
    ...
    try {
      ...
      const linguagens = detectarLinguagens(resposta);
      const resultados = await criarResultadosWhatweb(this.montarResultados(linguagens, metadados, resposta));
  ...
  private montarResultados(linguagens: { nome: string; evidencias: string[] }[], metadados: MetadadosTecnologia, resposta: { url: string; cabecalhos: Record<string, string>; caminho: string }): ResultadoWhatweb[] {
    const base = {
    ...
    return linguagens.map((linguagem) => ({
      plugin: 'Linguagem',
      valor: linguagem.nome,
      dados: { evidencias: linguagem.evidencias, url: resposta.url, caminho: resposta.caminho, cabecalhos: resposta.cabecalhos },
      ...base
    }));
  }
